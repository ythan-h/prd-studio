...
export default async function handler(req, res) {
  ...
  if (!process.env.OPENROUTER_API_KEY) {
    console.error('[critique-prd] OPENROUTER_API_KEY is not set');
  ...
  try {
    ...
    if (error.status === 429) {
      return res.status(429).json({ error: 'Rate limit reached on the free tier. Wait 60s and try again.' });
    }
    if (error.status === 402) {
      return res.status(402).json({
        error: 'Free tier limit reached on OpenRouter. Try again tomorrow or add $10 lifetime credits at openrouter.ai.',
      });
    }
