...
async function callWithFallback({ system, user, maxTokens }) {
  ...
  for (const model of MODEL_CHAIN) {
    try {
      const content = await callModel({ system, user, maxTokens, model });
      return parseJSON(content);
    } catch (err) {
      lastErr = err;
      const retryableStatus = err.status === 429 || err.status === 503 || err.status === 404 || err.status === 504;
      const parseFailure = err instanceof SyntaxError;
      if (!retryableStatus && !parseFailure) throw err;
      const reason = parseFailure ? 'malformed JSON' : `HTTP ${err.status}`;
      console.warn(`[aiService] Model ${model} failed (${reason}), trying next in chain`);
    }
  }
  throw lastErr;
}
